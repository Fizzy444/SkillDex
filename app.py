...
def chat_api():
    try:
        ...
        if not message and not uploaded_file:
            return jsonify({"error": "Please provide a message or upload an image/PDF."}), 400

        if session_id not in chat_sessions:
            chat_sessions[session_id] = model.start_chat(
                history=[]
            )
            chat_sessions[session_id].send_message(system_message)

        file_part = None
        if uploaded_file:
            filename = uploaded_file.filename
            mime_type = uploaded_file.mimetype
            print(f"Processing file: {filename}, MIME={mime_type}")

            if not (mime_type.startswith("image/") or mime_type == "application/pdf"):
                return jsonify({"error": "Unsupported file type. Please upload an image or PDF."}), 400

            if uploaded_file.content_length and uploaded_file.content_length > 16 * 1024 * 1024:
        ...
        if not message:
            message = "Please analyze this document in detail."
